...
// proper-lockfile removed: Bun (OpenCode's internal runtime) cannot import CJS
// modules with no default export via ESM. Replaced with an in-memory async mutex
// which is sufficient for single-process desktop use.
...
async function withFileLock<T>(path: string, fn: () => Promise<T>): Promise<T> {
  ...
  _fileLocks.set(path, prev.then(() => current));
  try {
    await prev;
    return await fn();
  } finally {
    resolve();
    if (_fileLocks.get(path) === current) _fileLocks.delete(path);
  }
}
